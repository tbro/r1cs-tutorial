...
impl TransactionVar {
    ...
    pub fn validate(
        ...
        post_sender_acc_info.balance = post_sender_acc_info
        ...
        post_recipient_acc_info.balance = post_recipient_acc_info
        ...
        let sender_exists = pre_sender_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, pre_root, &pre_sender_acc_info.to_bytes_le().as_slice());

        let sender_updated_correctly = post_sender_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, post_root, &post_sender_acc_info.to_bytes_le().as_slice());
        ...
        // information is correct with respect to `post_tx_root`.
        let recipient_exists = pre_recipient_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, pre_root, &pre_recipient_acc_info.to_bytes_le().as_slice())?;

        let recipient_updated_correctly = post_recipient_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, post_root, &post_recipient_acc_info.to_bytes_le().as_slice())?;
